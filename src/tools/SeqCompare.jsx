...
export default function SeqCompare({ setPage }) {
  ...
  return (
    <KitchenBench title="SeqCompare" kitchenTitle="Multi-FASTA Comparison" subtitle="Compare multiple FASTA sequence batches by exact sequence, ID, or full record." icon={<CuttingBoardIcon />}>
      ...
      {result && <><ResultPlate summary={result.summary} /><section className="panel chart-grid"><MiniStat label="Core" value={result.core.length} /><MiniStat label="Accessory" value={result.accessory.length} /><MiniStat label="File-specific" value={result.fileSpecific.length} /></section><DataTable rows={result.pairwise} caption="Pairwise file comparison (shared / unique sequence counts, Jaccard similarity)" csvName="seqcompare_pairwise" /><DataTable rows={result.globalKeys.map((g) => ({ key_id: g.globalKeyId, representative_id: g.representativeId, length: g.representativeLength, presence_count: g.presentInFiles.length, pattern: g.membershipPattern }))} caption="Global sequence keys and per-file presence pattern (length in bp/aa)" csvName="seqcompare_global_keys" /><SequencePreview text={result.exports.coreFasta} /><WarningPanel warnings={result.warnings} /><MethodRecipeCard methods={result.methods} settings={settings} /><ExportPantry tool="seqcompare" exports={result.exports} /><div className="send-row"><button className="button secondary" onClick={() => setPage("ReadLens")}>Send core FASTA to ReadLens</button><button className="button secondary" onClick={() => setPage("SeqSieve")}>Send accessory FASTA to SeqSieve</button><button className="button secondary" onClick={() => setPage("HMMForge")}>Send selected subset to HMMForge</button></div></>}
    </KitchenBench>
  );
}
